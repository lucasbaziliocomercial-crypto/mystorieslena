/**
 * PROMPT FALLBACK (Romance de Máfia) — extrai bloco <erros_detalhados> de
 * um markdown de revisão que NÃO veio com o XML estruturado.
 *
 * Versão Máfia difere da milionário no mapeamento de emojis:
 *   🟢 → naoInterfere · 🟡 → atencao · 🔴 → interfere · 💀 → gravissimo
 * (a versão milionário usa 🟠 pra interfere e 🔴 pra gravissimo).
 */
...
Mapeamento de gravidade (Romance de Máfia):
- 🟢 → "naoInterfere"
- 🟡 → "atencao"
- 🔴 → "interfere"
- 💀 → "gravissimo"
