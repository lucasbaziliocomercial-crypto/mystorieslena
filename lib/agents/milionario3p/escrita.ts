 ...
 * observado de fora pelos atos); PARTE 2 em terceira pessoa ONISCIENTE (o
 * narrador acessa a mente da FMC E do MMC). Em ambas: nenhum personagem narra
 * em 1ª pessoa, sem POV alternado / ✦ NOME.
 */
export const escritaAgent: Agent = {
  ...
  effort: "low",
  systemPrompt:
    ESCRITA_SYSTEM_PROMPT + CANONE_RULE + TENSE_RULE + JUNCTION_CONTINUITY_RULE,
    ...
    if (ctx.refineMode && ctx.currentOutput?.trim() && ajustes) {
      ...
      refine.push(
        [
          ...
          "• NARRAÇÃO em TERCEIRA PESSOA, com recorte por parte: PARTE 1 limitada à FMC (MMC observado de fora pelos atos, sem entrar na mente dele); PARTE 2 onisciente (o narrador acessa a mente da FMC E do MMC). Em ambas: nenhum personagem narra; NUNCA primeira pessoa na narração; sem ✦ NOME.",
    ...
    if (batch) {
      ...
      const narrationLabel =
        batch.part === "Parte 1"
          ...
          : "narrador externo em terceira pessoa ONISCIENTE (acessa a mente da FMC E do MMC; nenhum personagem narra; sem ✦ NOME)";
        ...
        batch.chapterTargets && batch.chapterTargets.length === batch.chapters.length
          ? batch.chapters
              .map((n, i) => {
                ...
                // Mira NO alvo (×1,00 — era ×0,97): mesmo motivo das demais
                // categorias (o dado mostrou que a maioria das reescritas de
                // calibração era EXPANSÃO — o modelo SUBESCREVE a mira). O TETO
                // (+3%) barra o excesso e o balanço de total fecha a SOMA. A 3p
                // sobe só ATÉ o alvo (não acima) por ser a categoria que
                // historicamente mirava mais baixo.
                const aim = Math.round(t * 1.0);
