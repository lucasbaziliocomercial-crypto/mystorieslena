 ...
 *
 * System prompt = master Alpha King + estrutura1 Alpha King.
 */
export const estrutura1Agent: Agent = {
  ...
  buildUserMessage: (ctx) => {
    ...
    sections.push(
      "Você vai montar a ESTRUTURA da PARTE 1 da história werewolf Alpha King. Toda a regra está no seu system prompt — siga na risca, sem alucinar e sem desvios. A premissa abaixo é a fonte primária do conteúdo. Use o que estiver no campo de instruções adicionais como ajuste opcional.",
    );

    if (ctx.referenceImage) {
      sections.push(
        "━━━ IMAGEM DE REFERÊNCIA ANEXADA ━━━\n\nA roteirista anexou uma imagem visual de referência. USE essa imagem pra calibrar mood/atmosfera (luz, paleta, peso ancestral do reino lobo), aparência física dos personagens (especialmente o Alpha King e o lobo dele), cenário/ambientação (castelo, alcateia, floresta, clareira sagrada), estilo visual. Integre os elementos visuais à estrutura sem inventar contradições com a premissa textual. Em conflito, a PREMISSA TEXTUAL prevalece.",
      );
    }
    ...
    if (titulo) {
      ...
      sections.push(
        [
          ...
          ``,
          `A premissa não trouxe uma linha "TÍTULO PROVISÓRIO:". CRIE você mesmo um título provisório comercial, curto e sedutor com peso werewolf a partir da premissa, e trate-o como o TÍTULO OFICIAL desta história.`,
          ``,
          `⚠️ REGRA INEGOCIÁVEL:`,
