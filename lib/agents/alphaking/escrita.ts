        ...
        batch.chapterTargets && batch.chapterTargets.length === batch.chapters.length
          ? batch.chapters
              .map((n, i) => {
                ...
                // Mira LEVEMENTE ACIMA do alvo (×1,02 — era ×0,99): o dado de
                // produção (perf.jsonl, 69 gerações) mostrou que 81% das reescritas
                // de calibração eram EXPANSÃO — o modelo SUBESCREVE a mira, então
                // mirar abaixo do alvo fazia a maioria dos caps nascer curta e
                // gastar reescrita (↑) pra crescer. Mirar ×1,02 centra a 1ª passada
                // perto do alvo; o TETO (+3%) barra o excesso e o balanço de total
                // fecha a SOMA. Reduz o nº de reescritas de calibração.
                const aim = Math.round(t * 1.02);
