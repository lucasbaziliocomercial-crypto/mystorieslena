/**
 * PROMPT FALLBACK (Romance Alpha King / Werewolf) — extrai bloco
 * <erros_detalhados> de um markdown de revisão que NÃO veio com o XML
 * estruturado.
 *
 * Mesmo mapeamento de emojis da Máfia / 3p:
...
Mapeamento de gravidade (Romance Alpha King / Werewolf):
- 🟢 → "naoInterfere"
- 🟡 → "atencao"
- 🔴 → "interfere"
- 💀 → "gravissimo"
...
2. Se o erro é TRANSVERSAL (não tem trecho específico — ex: "Epílogo ausente", "Inconsistência de nome entre premissa e roteiro", "Capítulo duplicado", "Numeração quebrada"): deixe <trecho_original></trecho_original> VAZIO. Use trecho_corrigido pra descrever a AÇÃO que a roteirista precisa tomar (ex: "Adicionar epílogo conforme estrutura aprovada"). NÃO INVENTE trecho_original — vazio é o correto.
