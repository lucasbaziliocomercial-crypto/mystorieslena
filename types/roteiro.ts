...
// NOTA: o step "overview" (Overview Final) foi removido do FLUXO — não entra
// mais no STEP_ORDER nem no stepper, então o wizard termina no Revisor. O
// StepId "overview" e seus agentes/rota continuam no código (inativos) para
// não quebrar tipos/imports e preservar roteiros legados que já tenham um
// output de overview salvo.
...
/**
 * Nós de EXIBIÇÃO do stepper (5) — colapsa `revisor1`+`revisor2` num único nó
 * "Revisor" (o step abre com abas Parte 1 / Parte 2). O modelo de dados continua
 * com os 6 `StepId` de `STEP_ORDER` — isto é só apresentação. `steps` lista
 * os `StepId` que aquele nó representa; o primeiro é o destino do clique.
 * (O StepId "overview" ainda existe no tipo, mas saiu do fluxo — ver STEP_ORDER.)
 */
 ...
 *  cabeçalho "Etapa X de Y" bater com os 5 nós colapsados do stepper. */
