...
**Revisor — modo enxuto nas re-rodadas:** a 1ª revisão de cada Parte entrega o relatório completo; a partir da 2ª passada (re-geração com relatório anterior presente, ou "Continuar revisão") o Revisor roda em **modo enxuto** (`leanRevisorReport`) — entrega só `PRINCIPAIS ERROS` + o bloco `<erros_detalhados>` (que move os cards de 1 clique) e **pula** o ensaio (Sugestões/Análise de Leitor/Análise de Hater/Risco/Nota/Melhorias). Como output domina o wall-clock, isso ~corta o tempo da ação que as roteiristas mais repetem (≥3×/Parte). A instrução de override é injetada uma vez na factory `lib/agents/_shared/build-revisor-agent.ts` (cobre as 4 categorias); o disparo vive em `lib/generation/run-step.ts#runRevisorStep` (regenerar — detecta relatório anterior via `outputs[step]` não-sentinela ou `history[step]`) e no branch "Continuar revisão" do `StepShell` (sempre enxuto) — **mantenha os dois em sincronia**.
